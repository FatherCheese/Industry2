...
class GuiCannery(inventory: InventoryPlayer, private val tileEntity: TileEntityCannery) : GuiContainer(ContainerCannery(inventory, tileEntity)) {
    ...
    override fun drawGuiContainerForegroundLayer() {
        ...
        fontRenderer.drawString("Canning Machine", 46, 6, 4210752)
    ...
    override fun clickInventory(x: Int, y: Int, mouseButton: Int) {
        ...
        val isIngredient: Boolean = RecipesCannery.getRecipeList().containsKey(clickedItemId)

        if (inventorySlots is ContainerCannery) {
