...
class GuiInductionFurnace(inventory: InventoryPlayer, private val tileEntity: TileEntityInductionFurnace) :
    GuiContainer(ContainerInductionFurnace(inventory, tileEntity)) {
    ...
    override fun drawGuiContainerForegroundLayer() {
        ...
        fontRenderer.drawString("Induction Furnace", 64, 6, 4210752)
    ...
    override fun clickInventory(x: Int, y: Int, mouseButton: Int) {
        ...
        val isIngredient: Boolean = RecipesFurnace.smelting().smeltingList.containsKey(clickedItemId)
        val isRedstone: Boolean = AdvancedRedstoneFuel.getRedstoneFuelList().containsKey(clickedItemId)

        if (inventorySlots is ContainerInductionFurnace) {
