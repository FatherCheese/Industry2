...
class GuiExtractorCentrifuge(inventory: InventoryPlayer, private val tileEntity: TileEntityCentrifugeExtractor) :
    GuiContainer(ContainerExtractorCentrifuge(inventory, tileEntity)) {
    ...
    override fun drawGuiContainerForegroundLayer() {
        ...
        fontRenderer.drawString("Centrifuge Extractor", 64, 6, 4210752)
    ...
    override fun clickInventory(x: Int, y: Int, mouseButton: Int) {
        ...
        val isIngredient: Boolean = RecipesExtractor.getRecipeList().containsKey(clickedItemId)
        val isRedstone: Boolean = AdvancedRedstoneFuel.getRedstoneFuelList().containsKey(clickedItemId)

        if (inventorySlots is ContainerExtractorCentrifuge) {
