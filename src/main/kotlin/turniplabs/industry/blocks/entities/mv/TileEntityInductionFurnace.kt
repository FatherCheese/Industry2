...
import turniplabs.industry.blocks.machines.mv.BlockFurnaceInduction
import turniplabs.industry.recipes.fuels.AdvancedRedstoneFuel

class TileEntityInductionFurnace : TileEntityEnergyConductorDamageable(), IInventory {
    ...
    private fun isProducible(itemStack: ItemStack?): Boolean {
        return RecipesFurnace.smelting().smeltingList.containsKey(itemStack!!.item.id)
    }
    ...
    private fun canProduceFirst(): Boolean {
        ...
        if (isProducible(contents[2])) {
            val resultStack: ItemStack? = RecipesFurnace.smelting().getSmeltingResult(contents[2]!!.item.id)
    ...
    private fun canProduceSecond(): Boolean {
        ...
        if (isProducible(contents[3])) {
            val resultStack: ItemStack? = RecipesFurnace.smelting().getSmeltingResult(contents[3]!!.item.id)
    ...
    private fun produceFirstItem() {
        if (canProduceFirst()) {
            val itemStack: ItemStack = RecipesFurnace.smelting().getSmeltingResult(contents[2]!!.item.id) ?: return
    ...
    private fun produceSecondItem() {
        if (canProduceSecond()) {
            val itemStack: ItemStack = RecipesFurnace.smelting().getSmeltingResult(contents[3]!!.item.id) ?: return
    ...
    override fun updateEntity() {
        ...
        if (!worldObj.isClientSide) {
            if (worldObj.getBlockId(xCoord, yCoord, zCoord) != IndustryBlocks.advancedMachineFurnace.id &&
                currentMachineTime == 0 &&
                contents[2] == null || contents[3] == null) {
                BlockFurnaceInduction.updateBlockState(true, worldObj, xCoord, yCoord, zCoord)
                machineUpdated = true
            }
        }
