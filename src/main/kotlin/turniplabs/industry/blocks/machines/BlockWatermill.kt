...
class BlockWatermill(key: String?, id: Int, material: Material?) : BlockTileEntity(key, id, material) {
    private var keepInventory = false

    override fun getNewBlockEntity(): TileEntity {
        return TileEntityWatermill()
    }

    override fun blockActivated(world: World?, x: Int, y: Int, z: Int, player: EntityPlayer?): Boolean {
        if (!world?.isClientSide!!) {
            val tileEntity: TileEntityWatermill = world.getBlockTileEntity(x, y, z) as TileEntityWatermill
