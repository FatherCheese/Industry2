...
class BlockExtractorCentrifuge(key: String?, id: Int, material: Material?) : BlockTileEntityRotatable(key, id, material) {
    private var keepInventory = false

    private val machineTexture: Array<IntArray> = arrayOf(
        TextureHelper.getOrCreateBlockTexture(Industry2.MOD_ID, "advanced_machine_extractor.png"),
        TextureHelper.getOrCreateBlockTexture(Industry2.MOD_ID, "advanced_machine_extractor_on.png"),
    ...
    override fun getNewBlockEntity(): TileEntity {
        return TileEntityCentrifugeExtractor()
    }

    override fun blockActivated(world: World?, x: Int, y: Int, z: Int, player: EntityPlayer?): Boolean {
        if (!world!!.isClientSide) {
            val tileEntity: TileEntityCentrifugeExtractor = world.getBlockTileEntity(x, y, z) as TileEntityCentrifugeExtractor
    ...
    override fun getBlockTexture(blockAccess: WorldSource?, x: Int, y: Int, z: Int, side: Side): Int {
        ...
        val tileEntity: TileEntityCentrifugeExtractor = blockAccess?.getBlockTileEntity(x, y, z) as TileEntityCentrifugeExtractor
    ...
    companion object {
        private var instance: BlockExtractorCentrifuge? = null

        private fun setupInstance(machine: BlockExtractorCentrifuge) {
            instance = machine
        }

        private fun getInstance(): BlockExtractorCentrifuge {
