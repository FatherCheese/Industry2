...
import turniplabs.industry.blocks.entities.mv.TileEntityInductionFurnace

class BlockFurnaceInduction(key: String?, id: Int, material: Material?) : BlockTileEntityRotatable(key, id, material) {
    private var keepInventory = false

    private val machineTexture: Array<IntArray> = arrayOf(
        TextureHelper.getOrCreateBlockTexture(Industry2.MOD_ID, "advanced_machine_furnace.png"),
        TextureHelper.getOrCreateBlockTexture(Industry2.MOD_ID, "advanced_machine_furnace_on.png"),
    ...
    override fun getNewBlockEntity(): TileEntity {
        return TileEntityInductionFurnace()
    }

    override fun blockActivated(world: World?, x: Int, y: Int, z: Int, player: EntityPlayer?): Boolean {
        if (!world!!.isClientSide) {
            val tileEntity: TileEntityInductionFurnace = world.getBlockTileEntity(x, y, z) as TileEntityInductionFurnace
    ...
    override fun getBlockTexture(blockAccess: WorldSource?, x: Int, y: Int, z: Int, side: Side): Int {
        ...
        val tileEntity: TileEntityInductionFurnace = blockAccess?.getBlockTileEntity(x, y, z) as TileEntityInductionFurnace
    ...
    companion object {
        private var instance: BlockFurnaceInduction? = null

        private fun setupInstance(machine: BlockFurnaceInduction) {
            instance = machine
        }

        private fun getInstance(): BlockFurnaceInduction {
            return instance ?: throw NullPointerException("Instance of BlockFurnaceInduction hasn't been setup!")
        }
