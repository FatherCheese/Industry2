...
class BlockElectricFurnace(key: String?, id: Int, material: Material?) : BlockTileEntityRotatable(key, id, material) {
    ...
    private val machineTexture: Array<IntArray> = arrayOf(
        TextureHelper.registerBlockTexture(Industry2.MOD_ID, "machine_furnace.png"),
        TextureHelper.registerBlockTexture(Industry2.MOD_ID, "machine_furnace_on.png"),
        TextureHelper.registerBlockTexture(Industry2.MOD_ID, "machine_casing_basic.png")
    )
    ...
    override fun blockActivated(world: World?, x: Int, y: Int, z: Int, player: EntityPlayer?): Boolean {
        if (!world?.isClientSide!!) {
            ...
            EnergyAPI.displayGui(
                player, GuiElectricFurnace(player?.inventory, tileEntity),
    ...
    companion object {
        ...
        fun setupInstance(machine: BlockElectricFurnace) {
            instance = machine
        }
