    ...
    public void updateEntity() {
        ...
        if (!worldObj.isClientSide) {
            if (currentMachineTime == 0 || currentMachineTime > 0 && contents[2] != null || contents[3] != null) {
                BlockAdvancedExtractor.updateBlockState(true, worldObj, xCoord, yCoord, zCoord);
                onInventoryChanged();
            ...
            if (active)
                worldObj.notifyBlockChange(xCoord, yCoord, zCoord, IndustryBlocks.advancedMachineExtractor.id);
        }
    }
}
